...
const OwnersTab = async ({
    ...
    const groupingMembers: GroupingGroupMember[] = search
        ? (await searchGroupingMembers(`${groupingPath}:owners`, search)).members
        : (await ownedGrouping([`${groupingPath}:owners`], page, pageSize, mapSortBy(sortBy), isAscending))
              .groupingOwners.members;

    return (
        <GroupingMembersTable
            groupingMembers={groupingMembers}
            groupingPath={groupingPath}
            groupingExtension="owners"
        />
    );
};
