...
import { faUsers, faUserPlus, faUserMinus, faCrown, faShareAlt, faCog, faTools } from '@fortawesome/free-solid-svg-icons';
...
const SideNav = ({ selectedGrouping }: { selectedGrouping: string }) => {
    ...
    const links = [
        { href: 'all', icon: faUsers, label: 'List all members (Basis + Include - Exclude members)' },
                        ...
                        <li key={href} className="pr-0 pl-0 mt-1 mx-auto pt-0">
                            <TooltipProvider>
                                <Tooltip>
                                    <TooltipTrigger>
                                        <Link
                                            href={`/groupings/${decodeURIComponent(selectedGrouping)}/${href}`}
                                            className={`flex items-center justify-center w-12 h-12 rounded-full border-none ${isSelected ? 'bg-white' : 'bg-transparent'}`}
                                        >
                                            <FontAwesomeIcon aria-hidden="true" icon={icon} className="w-5 h-5"/>
                                        </Link>
                                    </TooltipTrigger>
                                    <TooltipContent className="bg-black text-white max-w-48 text-center" side="right">{label}</TooltipContent>
                                </Tooltip>
                            </TooltipProvider>
                        </li>
                    );
