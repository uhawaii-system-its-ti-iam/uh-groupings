...
type RemoveMemberTrashcanProps = {
    uid: string;
    name: string;
    uhUuid: string;
    group: string;

    // any provided action to run on confirm
    action: (uid: string) => void | Promise<void>;
        ...
        <AlertDialog open={open} onOpenChange={setOpen}>
            <AlertDialogTrigger asChild>
                <Trash2Icon
                    ...
                    className={iconClassName ?? 'h-4 w-4 text-red-600 cursor-pointer'}
            ...
            <AlertDialogContent className="sm:max-w-[500px]">
                ...
                <div className="px-3">
                    <Alert className="bg-yellow-100 border border-yellow-200 mb-2">
                        <AlertDescription>
                            Membership changes made may not take effect immediately. Usually, 3-5 minutes should be anticipated. In
                            extreme cases changes may take several hours to be fully processed, depending on the number of members
                            and the synchronization destination.
                        </AlertDescription>
                    </Alert>
                </div>
                ...
                <AlertDialogFooter>
                    <Button onClick={handleConfirm} disabled={submitting}>
                        {submitting ? 'Removing…' : 'Yes'}
                    </Button>
                    <AlertDialogCancel onClick={() => setOpen(false)} disabled={submitting}>
                        Cancel
                    </AlertDialogCancel>
                </AlertDialogFooter>
            </AlertDialogContent>
