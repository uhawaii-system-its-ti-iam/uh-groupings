/**
 * Tests for the <ListManagement /> client component (Include / Exclude / Owners panes).
 *
 * Component summary:
 *   - Renders the textarea for entering UH usernames / UH numbers plus the
 *     Remove/Manage buttons used to add or remove members from a grouping list.
 *   - The `list` prop is one of "include" | "exclude" | "owners" and selects
 *     which `getMembersExistIn{List}` action is invoked.
 *   - Validates raw input (rejects empty, whitespace-only, commas-only, and
 *     non-alphanumeric characters), detects duplicates, and routes the user to
 *     either the single-member or multiple-members manage modal via the
 *     `onOpenManageMemberModal` / `onOpenManageMembersModal` callbacks.
 *
 * Suite organization (mirror this when adding tests):
 *   - "Remove Button Click with Empty Inputs"             — input validation.
 *   - "Remove Button Click with Invalid Raw Text Inputs"  — character-set validation.
 *   - "ListManagement - Members In/Not In <List> Validation" — per-list happy path
 *     and not-in-list error messaging (one block per list).
 *   - "Remove Button Click with Duplicate Members in Input" — duplicate detection.
 *   - "List Management Removal Precedence"                — interaction between
 *     typed input and pre-checked members.
 *
 * Note: the actions module is mocked with explicit `vi.fn()`s rather than
 * `vi.mock('@/lib/actions')` because tests rely on the default `[]` resolved
 * value and only override per-test as needed via `vi.mocked(...).mockResolvedValue(...)`.
 */
