/**
 * Tests for the <Actions /> client component (Grouping Actions tab).
 *
 * Component summary:
 *   - Lets a grouping owner reset the Include and/or Exclude lists.
 *   - User selects one/both checkboxes then clicks "Reset Selected" then confirms in a modal
 *     then sees a loading spinner while the request is in flight then sees a completion modal.
 *   - The component picks between the sync (`resetIncludeGroup`, `resetExcludeGroup`)
 *     and async (`...GroupAsync`) variants based on path length to avoid request timeouts.
 *   - It also surfaces "duplicate owners" from server-provided initial props.
 *
 * Testing conventions:
 *   - `next/navigation` is mocked at the very top because the component reads
 *     `useRouter` at import time. A shared `refreshMock` allows asserting that
 *     the router was refreshed after a successful reset.
 *   - `@/lib/actions` is auto-mocked; use `vi.mocked(fn).mockImplementation(...)`
 *     or `.mockResolvedValue(...)` per test.
 *   - The spinner / status region intentionally produces multiple `role="status"`
 *     elements (outer wrapper + inner spinner), so we use `getAllByRole('status')`
 *     and `queryAllByRole('status').toHaveLength(0)` rather than `getByRole`.
 *   - Locate the form via `data-testid="grouping-actions-form"` and the spinner
 *     via `data-testid="loading-spinner"` — both are exposed by the component
 *     specifically for these tests.
 */

// IMPORTANT: vi.mock is hoisted, so this block must run before any imports
// that transitively use `useRouter` (which the component does at module load).
...
import type { GroupingRemoveResults } from '@/lib/types';

vi.mock('@/lib/actions');

/**
 * Build a minimal `GroupingRemoveResults` for mocks. The component only reads
 * `resultCode`, so callers usually only override that field; the other required
 * fields are filled with inert defaults to satisfy the type.
 */
const removeResults = (overrides: Partial<GroupingRemoveResults> = {}): GroupingRemoveResults => ({
    resultCode: 'SUCCESS',
    groupPath: 'mock:path',
    results: [],
    ...overrides
...
describe('Actions Component', () => {
    ...
    beforeEach(() => {
        vi.clearAllMocks();
        // `vi.clearAllMocks()` resets call history on `vi.fn()` instances declared
        // in this file (like `refreshMock`) as well, so each test starts clean.
    });
    ...
    it('shows spinner after confirming reset, then displays success modal with Ok button', async () => {
        vi.mocked(resetIncludeGroup).mockImplementation(() =>
            new Promise((resolve) => {
                setTimeout(() => resolve(removeResults({ resultCode: 'SUCCESS' })), 500);
            })
        );
        render(<Actions groupingPath={shortGroupingPath} />);
        const user = userEvent.setup();
    ...
    it('shows spinner after confirming reset, then displays success modal for resetExcludeGroup', async () => {
        vi.mocked(resetExcludeGroup).mockImplementation(() =>
            new Promise((resolve) => {
                setTimeout(() => resolve(removeResults({ resultCode: 'SUCCESS' })), 500);
            })
        );
    ...
    it('shows spinner after confirming reset, then displays success modal for resetExcludeGroupAsync', async () => {
        vi.mocked(resetExcludeGroupAsync).mockImplementation(() =>
            new Promise((resolve) => {
                setTimeout(() => resolve(removeResults({ resultCode: 'SUCCESS' })), 500);
            })
        );
    ...
    it('submits the form and triggers preventDefault to prevent page reload', () => {
        ...
        const form = screen.getByTestId('grouping-actions-form');
    ...
    it('shows spinner after confirming reset, then displays success modal for resetIncludeGroupAsync', async () => {
        vi.mocked(resetIncludeGroupAsync).mockImplementation(() =>
            Promise.resolve(removeResults({ resultCode: 'SUCCESS' }))
        );
    ...
    it('does not show success modal or call refresh if resetExcludeGroupAsync fails', async () => {
        vi.mocked(resetExcludeGroupAsync).mockImplementation(() =>
            Promise.resolve(removeResults({ resultCode: 'FAILURE' }))
        );
    ...
    it('does not show success modal or call refresh if resetIncludeGroup fails', async () => {
        vi.mocked(resetIncludeGroup).mockResolvedValue(removeResults({ resultCode: 'FAILURE' }));
    ...
    describe('Duplicate Owners Display', () => {
        ...
        it('has correct table styling with hover effect', () => {
            ...
            // Skip header row (index 0) to get body rows
            const allRows = within(screen.getByRole('table')).getAllByRole('row');
            const bodyRows = allRows.slice(1);
            expect(bodyRows.length).toBe(1);
            expect(bodyRows[0].className).toContain('hover:bg-gray-200');
            expect(bodyRows[0].className).toContain('bg-gray-50');
        });
    });
